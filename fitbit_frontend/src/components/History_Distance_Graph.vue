<template>
  <q-card class='full-width'>
    <q-card-section>
      <div class="text-h6">History Distance data for {{$store.state.main.view_user}}</div>
    </q-card-section>
    <q-separator />
    ...
    <q-card-section class='no-padding'>
      <div class='graphDistance' ref='chartdiv' />
    </q-card-section>
  </q-card>
</template>
  ...
  methods: {
    clickSearch(){
      ...
      this.$axios.post(this.$store.state.main.domain + '/retrieve_history_metrics',data,config).then(response => {
        if (response.data.error === '0') {
          ...
          for (var j=0;j<unique_dates.length;j++){
            ...
            chart.data.push({"Distance":sum,"Date":unique_dates[j]});
          }
      ...
      function update(){
        ...
        series.name = "Distance (Km/h)";
        series.tooltipText = "{name}: [bold]{valueY}[/]";
        series.strokeWidth = 3;
        series.stroke = am4core.color("#845EC2");
        series.tooltip.getFillFromObject = false;
        series.tooltip.background.fill = am4core.color("#845EC2");
